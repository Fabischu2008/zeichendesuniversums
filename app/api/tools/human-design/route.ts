...
import { buildHumanDesignResult } from "@/lib/tools/human-design";
...
export async function POST(req: Request) {
  try {
    ...
    const result = buildHumanDesignResult({
      dateUtc,
    ...
    return NextResponse.json({
      ok: true,
      result,
      meta: { tz, utc: utcIso, place: location.name },
    });
  } catch (e) {
    const message =
      e instanceof Error ? e.message : "Unbekannter Serverfehler.";
    return NextResponse.json(
      { message: `Serverfehler bei Human Design: ${message}` },
      { status: 500 },
    );
  }
}
