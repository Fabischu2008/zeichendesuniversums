...
import {
  buildAstrocartographyLines,
  buildAstroPlanetAnalyses,
  evaluateAstroMapQuality,
...
export async function POST(req: Request) {
  try {
    ...
    const lines = buildAstrocartographyLines(dateUtc);
    const analyses = buildAstroPlanetAnalyses(lines);
    const quality = evaluateAstroMapQuality({ dateUtc, lines });
    ...
    return NextResponse.json(
      { message: `Serverfehler bei Astro-Landkarte: ${message}` },
      { status: 500 },
    );
  }
}
